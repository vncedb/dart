...
export default function ReportDetailsScreen() {
    ...
    const executeDelete = async () => {
        ...
        try {
            ...
            if (user) {
                ...
                await db.runAsync('DELETE FROM accomplishments WHERE user_id = ? AND date = ?', [user.id, date]);
                if (report.attendanceId) {
                     await db.runAsync('INSERT INTO sync_queue (table_name, row_id, action, data) VALUES (?, ?, ?, ?)', ['attendance', report.attendanceId, 'DELETE', null]);
                }
                triggerSync();
                router.back();
            }
    ...
    const defaultTitleStyle = useAnimatedStyle(() => ({
        ...
        transform: [
            { scale: interpolate(scrollY.value, [20, 50], [1, 0.9], Extrapolation.CLAMP) }
        ]
    }));
    ...
    const renderTask = (acc: any, index: number) => (
        <View style={[styles.taskCard, { backgroundColor: theme.colors.card, borderColor: theme.colors.border }]}>
            ...
            <View style={styles.taskContent}>
                ...
                {acc.remarks ? <Text style={[styles.taskRemarks, { color: theme.colors.textSecondary }]}>{acc.remarks}</Text> : null}
            </View>
            ...
            {initialLoading ? (
                ...
                >
                    ...
                    <Animated.View style={[styles.dateHeader, useAnimatedStyle(() => ({ opacity: interpolate(scrollY.value, [0, 40], [1, 0], Extrapolation.CLAMP) })) ]}>
                        ...
                        <View>
                            <Text style={[styles.dayText, { color: theme.colors.text }]}>{format(new Date(report.date), 'EEEE')}</Text>
                            <Text style={[styles.dateText, { color: theme.colors.textSecondary }]}>{format(new Date(report.date), 'MMMM d, yyyy')}</Text>
                        </View>
                    </Animated.View>
                    ...
                    <View style={styles.statsContainer}>
                        <View style={[styles.statCard, { backgroundColor: theme.colors.card, borderColor: theme.colors.border }]}>
                            <View style={styles.statLabelRow}>
                                ...
                                <Text style={[styles.statLabel, { color: theme.colors.textSecondary }]}>Clock In</Text>
                            </View>
                            <Text style={[styles.statValue, { color: theme.colors.text }]}>{report.clockIn}</Text>
                        </View>
                        <View style={[styles.statCard, { backgroundColor: theme.colors.card, borderColor: theme.colors.border }]}>
                            <View style={styles.statLabelRow}>
                                <HugeiconsIcon icon={Clock01Icon} size={14} color={theme.colors.warning} />
                                <Text style={[styles.statLabel, { color: theme.colors.textSecondary }]}>Clock Out</Text>
                            </View>
                            <Text style={[styles.statValue, { color: theme.colors.text }]}>{report.clockOut}</Text>
                        </View>
                    </View>

                    <View style={styles.sectionHeader}>
                        <HugeiconsIcon icon={Task01Icon} size={18} color={theme.colors.text} />
                        ...
                        <View style={[styles.badge, { backgroundColor: theme.colors.card, borderColor: theme.colors.border }]}>
                            <Text style={[styles.badgeText, { color: theme.colors.textSecondary }]}>{report.accomplishments.length}</Text>
                        </View>
                    </View>
                    ...
                    {report.accomplishments.length === 0 ? (
                        <View style={styles.emptyState}>
                            <HugeiconsIcon icon={Time02Icon} size={40} color={theme.colors.icon} />
                            <Text style={{ color: theme.colors.textSecondary, marginTop: 12 }}>No activity recorded.</Text>
                        </View>
                    ) : (
                        <AnimatedList 
...
const styles = StyleSheet.create({
    ...
    calendarIcon: { width: 52, height: 52, borderRadius: 16, alignItems: 'center', justifyContent: 'center', marginRight: 16 },
    ...
    taskTitle: { fontSize: 16, fontWeight: '600', lineHeight: 22, marginBottom: 4 },
    taskRemarks: { fontSize: 14, lineHeight: 20, opacity: 0.7 },
    imageGrid: { marginTop: 16, flexDirection: 'row', flexWrap: 'wrap', gap: 10, marginLeft: 16 },
